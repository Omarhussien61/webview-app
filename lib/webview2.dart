...
class Web_View2 extends StatefulWidget {
  const Web_View2({Key? key, this.cookieManager, this.initialUrl}) : super(key: key);
    ...
    checkNewVersion() async {
      try {
        ...
        if (currentBuildNumber < requiredBuildNumber) {
          await _showUpdateDialog(remoteConfig.getBool('force'),remoteConfig.getString('message'));
        }
      } catch (e) {
        print(e.toString());
      }
                ...
                body: Container(
                  ...
                  child: Column(
                   ...
                   Container(
                        ...
                        child: FlatButton(
                          onPressed: () async {
                            String? updateURL =Strings.kUpgradeURLConfig[Platform.isAndroid ? 'android' : 'ios'];
                            try {
                          ...
                          shape: RoundedRectangleBorder(
                              side: BorderSide(color: Color(0xFFEB7E23), width: 1, style: BorderStyle.solid), borderRadius: BorderRadius.circular(3.0)),
                        ),
                      ),
                      SizedBox(height: 25,),
...
//    );
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Scaffold(
        ...
        centerTitle: true,
        leading:canback? IconButton(onPressed: (){
      controllerGlobal.goBack();
      }, icon: Icon(Icons.keyboard_arrow_right),) :Container(),
        actions: [
          IconButton(
            onPressed: (){
                  ...
                  navigationDelegate: (NavigationRequest request) async {
                   // String url ="https://stage.wiakum.com/pub/applepay.php?order_id=55555&amount=1.00";
                    ...
                    if (request.url.startsWith('https://www.youtube.com/')) {
                      ...
                      if (request.url.startsWith('https://wiakum.com/payments/order/applepay/')) {
                      ...
                      {
                        var result =await Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) { return Apple_Pay(order_id:"${uri.queryParameters['orderId']}", mount: '${uri.queryParameters['amount']}',); }));
                        if(result!=null) {
                     ...
                     return NavigationDecision.prevent;
                      ...
                      if (request.url.startsWith('https://api.whatsapp.com/send')||
                          request.url.startsWith('https://www.facebook.com/sharer/sharer')||
                          request.url.startsWith('https://twitter.com/intent/tweet?')||
                    ...
                    children: [
                      InkWell(
                        ...
                        child: Container(
                          ...
                          //   ],
                          ),child: Image.asset("assets/logo_icon.png",width: 50,height: 50,),),
                      ),
                      InkWell(
                        onTap: () async {
                          var result =await Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) { return MainCategoriesPage(); }));
                          if(result!=null) {
                            controllerGlobal.loadUrl(result);
                          }
                        },
  ...
  Future<void> _performtrxn(BuildContext context  ,amount,orderid) async {
    ...
    var act='1';
      ...
      controllerGlobal.loadUrl("https://wiakum.com/payments/order/applepay?orderId=${orderid}&amount=${amount}&payment_result=success&merchant_reference=1999&payment_id=4343434");
